...
def init_wandb(args, cfg):
    if cfg.get('SELF_TRAIN', None):
        ...
        adaptation_task = '{}'.format(
            cfg.DATA_CONFIG.DATASET.split('Data')[0][0])

    proj_name = "ReDB_{}_train".format(adaptation_task)
...
def main():
    ...
    if cfg.get('SELF_TRAIN', None):
        if cfg.SELF_TRAIN.LOAD_SCRATCH_AFTER_PSEUDO_LABELING \
                or cfg.SELF_TRAIN.LOAD_OPTIMIZER_AFTER_PSEUDO_LABELING:
            args.epochs = cfg.SELF_TRAIN.TOTAL_NUM_EPOCHS
    ...
    logger.info('dist_train: %s', str(dist_train))
    ...
    source_batch_size = 1 if cfg.get('SELF_TRAIN', None) else args.batch_size
